...
// url
const baseURL = "https://otoate-fukushima.herokuapp.com/"
...
const onLoadResultScene = async () => {
  ...
  // スコアの保存・取得
  try {
    const currentScore = await createScore({
      name,
      score
    })
    const data = await fetchScoreList()
    updateScoreList(data, currentScore)
  } catch (e) {
    console.error(e)
  }
}
...
const createScore = async (params) => {
  const data = await fetch(baseURL + '/scores', {
    ...
    // dataがjsonオブジェクトでURLにPOSTで送るための記述
